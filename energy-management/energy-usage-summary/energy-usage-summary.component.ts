...
@Component({
  ...
  styleUrls: [
    "./energy-usage-summary.component.scss",
...
export class EnergyUsageSummaryComponent implements OnInit, OnDestroy {
  ...
  retrieveEnergyUsageData() {
    if (this.dateStart && this.dateEnd && this.selectedPeriod) {
      ...
      this.restService
        .postData("getEnergyUsageSummary", this.authService.getToken(), {
        ...
        .subscribe((data) => {
          // Success
          if (data["status"] == 200) {
            this.generateEnergyUsageChartByAssetType(
              data["data"]["rows"]["assettype_kwhsummary"]
            );
            this.generateEnergyUsageChartByAssetName(
              data["data"]["rows"]["assetname_kwhsummary"]
            );
            this.generateEnergyUsageChartByControllerID(
              data["data"]["rows"]["controllername_kwhsummary"]
            );
          }
        });
    }
  ...
  ngOnInit() {
    ...
    this.restService
      .postData("getEnergyMinMaxDate", this.authService.getToken())
      .subscribe((data) => {
        // Success
        if (data["status"] == 200) {

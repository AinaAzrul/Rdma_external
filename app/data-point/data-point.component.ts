...
export class DataPointComponent implements OnInit {
    ...
    reader.onload = (e: any) => {
      ...
      if (wb.SheetNames.length > 1){
        for (const  item of wb.SheetNames) {
          ...
          objSheet['data'] = <AOA>(XLSX.utils.sheet_to_json(tempData, { header:1, raw:false, defval:null}));
 ...
 /* onSubmit() {
  ...
  onSubmit() {
    this.myObj = JSON.stringify(this.multipleTabData);
    this.Obj = JSON.parse(this.myObj);
    this.restService.postData("addDataPoint", this.authService.getToken(), 
    {facility_name:this.uploadDataForm.value.facilityName, file_name:this.excelName,
      file_sheets:this.myObj,service_name: this.uploadDataForm.value.services
    })
    .subscribe(data => {
      // Success
      if (data["status"] == 200) {
